...
def test_aes_128_key_wrap():
    """
    Test the algorithms with example data from RFC3394
    """
    wrapping_key = '000102030405060708090A0B0C0D0E0F'
    wrapping_key_bytes = bytes.fromhex(wrapping_key)
    ss = SecuritySuite0(wrapping_key_bytes)

    key_to_wrap = '00112233445566778899AABBCCDDEEFF'
    key_to_wrap_bytes = bytes.fromhex(key_to_wrap)

    wrapped_key = '1FA68B0A8112B447AEF34BD8FB5A7B829D3E862371D2CFE5'
    wrapped_key_bytes = bytes.fromhex(wrapped_key)

    # wrap the key
    wrapped_key_result_bytes = ss.wrap_key(wrapping_key_bytes, key_to_wrap_bytes)

    assert wrapped_key_result_bytes == wrapped_key_bytes

    # unwrap the key
    unwrapped_key_result_bytes = ss.unwrap_key(wrapping_key_bytes, wrapped_key_result_bytes)

    assert unwrapped_key_result_bytes == key_to_wrap_bytes


def test_aes_gcm_128_decrypt():

    initialization_vector = b'/\x19\x06\x91\x99\x16A\x03\x00\x00\x00\x0e'
    key = b'MYDUMMYGLOBALKEY'
    data = b'S\xb0\x16Q\xc7\xc0\x9c\x81&:\xb8\xae\xbb\xb1?\xe9. =Hh"\xb2.\x9evT\x16k\xec\xb4\xef\xc0)\x1d\xf0\xab\xd3?\xd9igy\x93\\~\xb1\x94\xba6<\xa5\xfe\xd4'
    ss = SecuritySuite0(key)

    decrypted = ss.decrypt(initialization_vector, data_to_decrypt=data, associated_data=(b'0' + key))
    print(decrypted)
    assert decrypted == b'\x0f\x00\x00\x00\r\x00\t"\x12Z\x86\x8f\xc4\x00\x00\x00\x00\x18\x00\x00\x00\x00\x00\x00\x00\x00Z\x86eP\x04\x80\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00'  # To throw error with debug info
# first part is UDP-wrapper
